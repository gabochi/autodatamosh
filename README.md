...
Genera animaciones con efecto **datamosh real** (a nivel de bitstream) a partir de una sola imagen, usando solo `bash`, `ffmpeg`, `ffprobe` y `python3`.
...
Re-codificar con x264 **no** produce este efecto (x264 siempre predice bien y "arregla" el corte). El glitch requiere corrupción a nivel de bitstream: por eso este script une los streams comprimidos a mano.

La imagen original derecha **nunca** se muestra: la intro ya es una variante rotada y las secciones muestran la imagen volteada/girada/desplazada, cada una con un solo I-frame que se le quita. El movimiento dentro de cada sección es un **zoom pulsante in-out + barrido lateral** (sin zoom-in fijo), para que la imagen nunca quede quieta ni entera.
...
- `ffmpeg` y `ffprobe`
- `python3`
- `LC_ALL` forzado a `C` dentro del script (necesario para que los decimales funcionen en los filtros de ffmpeg)
...
- En modo `bleed` las variantes son rotaciones/volteos; en modo `melt` son desplazamientos grandes (con algunos volteos).
